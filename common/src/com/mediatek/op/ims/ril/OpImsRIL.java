...
import static com.mediatek.opcommon.telephony.MtkRILConstantsOp.RIL_REQUEST_DIAL_FROM;
import static com.mediatek.opcommon.telephony.MtkRILConstantsOp.RIL_REQUEST_SEND_USSI_FROM;
import static com.mediatek.opcommon.telephony.MtkRILConstantsOp.RIL_REQUEST_CANCEL_USSI_FROM;
import static com.mediatek.opcommon.telephony.MtkRILConstantsOp.RIL_REQUEST_SET_EMERGENCY_CALL_CONFIG;
import static com.mediatek.opcommon.telephony.MtkRILConstantsOp.RIL_REQUEST_DEVICE_SWITCH;
import static com.mediatek.opcommon.telephony.MtkRILConstantsOp.RIL_REQUEST_CANCEL_DEVICE_SWITCH;
...
import vendor.mediatek.hardware.radio_op.V3_0.DialFrom;
...
class RILRequest {
    ...
    onError(int error, Object ret) {
        ...
        if (OpImsRIL.OpImsRIL_LOGD) Rlog.d(LOG_TAG, serialString() + "< "
            + OpImsRIL.requestToString(mRequest)
            + " error: " + ex + " ret=" + OpImsRIL.retToString(mRequest, ret));
...
public final class OpImsRIL extends OpImsBaseCommands {
    ...
    static final boolean OpImsRIL_LOGV = false; // STOPSHIP if true
            ...
            switch (msg.what) {
                    ...
                    synchronized (mRequestList) {
                        if (msg.arg1 == mWlSequenceNum && clearWakeLock(FOR_WAKELOCK)) {
                            if (OpImsRIL_LOGD) {
                                ...
                                Rlog.d(OpImsRIL_LOG_TAG, "WAKE_LOCK_TIMEOUT " +
                                        " mRequestList=" + count);
                                for (int i = 0; i < count; i++) {
                                    rr = mRequestList.valueAt(i);
                                    Rlog.d(OpImsRIL_LOG_TAG, i + ": [" + rr.mSerial + "] "
                ...
                case EVENT_ACK_WAKE_LOCK_TIMEOUT:
                    if (msg.arg1 == mAckWlSequenceNum && clearWakeLock(FOR_ACK_WAKELOCK)) {
                        if (OpImsRIL_LOGV) {
                            Rlog.d(OpImsRIL_LOG_TAG, "ACK_WAKE_LOCK_TIMEOUT");
                        }
                    }
                    break;
    ...
    private IRadioOp getRadioProxy(Message result) {
        if (!mIsMobileNetworkSupported) {
            if (OpImsRIL_LOGV) riljLog("getRadioProxy: Not calling getService(): wifi-only");
            return null;
        }
    ...
    public OpImsRIL(Context context, int instanceId) {
        ...
        mIsMobileNetworkSupported = true; //cm.isNetworkSupported(ConnectivityManager.TYPE_MOBILE);

        mRadioResponse = new OpImsRadioResponse(this, instanceId);
        mRadioIndication = new OpImsRadioIndication(this, instanceId);

        mRilHandler = new RilHandler();
        mRadioProxyDeathRecipient = new RadioProxyDeathRecipient();
        PowerManager pm = (PowerManager)context.getSystemService(Context.POWER_SERVICE);
        mWakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, OpImsRIL_LOG_TAG);
        ...
        if (OpImsRIL_LOGD) {
            riljLog("Ims-RIL-WWOP: proxy = " + (proxy == null));
        }
    }
    ...
    public void dialFrom(String address, String fromAddress, int clirMode, boolean isVideoCall,
        ...
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(RIL_REQUEST_DIAL_FROM, response,
    ...
    public void sendUssiFrom(String from, int action, String ussi, Message response) {
        IRadioOp radioProxy = getRadioProxy(response);
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(RIL_REQUEST_SEND_USSI_FROM, response,
                    mRILDefaultWorkSource);
    ...
    public void cancelUssiFrom(String from, Message response) {
        IRadioOp radioProxy = getRadioProxy(response);
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(RIL_REQUEST_CANCEL_USSI_FROM, response,
                    mRILDefaultWorkSource);
    ...
    public void deviceSwitch(String number, String deviceId, Message response) {
        IRadioOp radioProxy = getRadioProxy(response);
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(RIL_REQUEST_DEVICE_SWITCH, response,
                    mRILDefaultWorkSource);
    ...
    public void cancelDeviceSwitch(Message response) {
        IRadioOp radioProxy = getRadioProxy(response);
        if (radioProxy != null) {
            RILRequest rr = obtainRequest(RIL_REQUEST_CANCEL_DEVICE_SWITCH, response,
                    mRILDefaultWorkSource);
    ...
    void processIndication(int indicationType) {
        if (indicationType == RadioIndicationType.UNSOLICITED_ACK_EXP) {
            sendAck();
            if (OpImsRIL_LOGD) riljLog("Unsol response received; Sending ack to ril.cpp");
        } else {
    ...
    void processRequestAck(int serial) {
        ...
        if (rr == null) {
            Rlog.w(OpImsRIL.OpImsRIL_LOG_TAG,
    ...
    RILRequest processResponse(RadioResponseInfo responseInfo) {
        ...
        rr = findAndRemoveRequestFromList(serial);
        if (rr == null) {
            Rlog.e(OpImsRIL_LOG_TAG, "processResponse: Unexpected response! serial: " + serial
                    + " error: " + error);
            return null;
        }
    ...
    private boolean clearWakeLock(int wakeLockType) {
        if (wakeLockType == FOR_WAKELOCK) {
            synchronized (mWakeLock) {
                ...
                Rlog.d(OpImsRIL_LOG_TAG, "NOTE: mWakeLockCount is " + mWakeLockCount
                        + "at time of clearing");
                mWakeLockCount = 0;
                mWakeLock.release();
    ...
    private void clearRequestList(int error, boolean loggable) {
        ...
        synchronized (mRequestList) {
            int count = mRequestList.size();
            if (OpImsRIL_LOGD && loggable) {
                Rlog.d(OpImsRIL_LOG_TAG, "clearRequestList " + " mWakeLockCount="
    ...
    void riljLog(String msg) {
        Rlog.d(OpImsRIL_LOG_TAG, msg
                + (mPhoneId != null ? (" [SUB" + mPhoneId + "]") : ""));
    }

    void riljLoge(String msg) {
        Rlog.e(OpImsRIL_LOG_TAG, msg
                + (mPhoneId != null ? (" [SUB" + mPhoneId + "]") : ""));
    }
